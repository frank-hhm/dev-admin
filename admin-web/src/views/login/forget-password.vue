<template>
    <a-modal title="找回密码" @BeforeOk="onCreateOk" @BeforeCancel="close" :width="isMobile ? 'calc(100% - 20px)' : '360px'"
        ...
        <a-form :model="createForm" layout="vertical" ref="createRef" :rules="createRules">
                ...
                <a-input v-model="createForm.captcha_code" placeholder="请输入验证码" allow-clear maxlength="4" size="large">
                    <template #append>
                        <a-spin :loading="captchaLoading">
                            <div @click="toLoginCaptcha" class="captcha-img">
            ...
            <a-form-item :label-col-flex="labelColFlex" label="邮箱验证码" field="code">
...
const getCode = () => {
    // return false;
    proxy?.$refs['createRef']?.validateField(['email', 'captcha_code'], (error: undefined | Record<string, ValidatedError>) => {
        if (error === undefined) {
...
const onCreateOk = () => {
    proxy?.$refs['createRef']?.validate((error: undefined | Record<string, ValidatedError>) => {
        if (error === undefined) {
            ...
            forgetPasswordApi(createForm.value).then((res: Result) => {
                $utils.successMsg(res.message);
                setTimeout(() => {
                    close()
                }, 300);
